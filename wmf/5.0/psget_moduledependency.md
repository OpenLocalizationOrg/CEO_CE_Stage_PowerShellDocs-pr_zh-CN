...
没有在安装模块中，通过并行 (SxS) 模块版本支持更新模块和运行在 Windows PowerShell 5.0 或更高版本的发布模块 cmdlet。
此外，我们已添加到指定要发布的版本发布模块 cmdlet 的-RequiredVersion 参数。 Path 参数现在支持模块基版本的文件夹的路径。
...
**内容 ModuleWithDependencies2 模块的清单文件︰**
```powershell
@{
