...
安装脚本 cmdlet，可以将特定的脚本文件以及其相关性安装到指定的范围。 默认情况下，脚本安装到 AllUsers 范围。 获取 InstalledScript cmdlet 允许您以获取使用安装脚本 cmdlet 已安装的脚本文件的列表。
...
## 更新脚本 cmdlet
更新脚本 cmdlet 允许您执行就地使用安装脚本 cmdlet 已安装的脚本文件的更新。
```powershell
...
## 新 ScriptFileInfo 和测试 ScriptFileInfo cmdlet
新 ScriptFileInfo cmdlet，您可以使用元数据，如版本、 Guid、 作者和说明，创建新的脚本文件等。测试 ScriptFileInfo cmdlet 可以验证并获取脚本文件元数据。
```powershell
...
## 带脚本共享支持的 register PSRepository 和设置 PSRepository cmdlet
使用 Register PSRepository 设置 PSRepository cmdlet 添加 PSRepository **ScriptSourceLocation**和**ScriptPublishLocation** 。
```powershell
...
## 发布脚本 cmdlet
发布脚本 cmdlet，您可以发布您的脚本文件具有有效的元数据，如版本、 Guid、 作者和说明，等等。
```powershell
